...
def depthFirstSearch(problem):
    ...
    path = set ( [stat] )                       # Assegurar-se en no orepetir els elements.
    ...
    while not problem.isGoalState ( stat ) and nextNode:
        ...
        node = nextNode.pop ()  # El següent node.
        stat = node.stat        # l'estat del node en questio

        # Només operarem si no està registrat, i evitarem fer calculs si és la solució desitjada.
        if not stat in path and not problem.isGoalState (stat):
            path.add (stat) # El registrem  per evitar repeticions.

            # Definim els següents passos a seguir.
            successors = problem.getSuccessors (stat)
            [ nextNode.append (Node ( nStat, node, [nAction], node.cost + nCost )) for nStat, nAction, nCost in successors if nStat not in path ]
...
def breadthFirstSearch(problem):
    ...
    while not problem.isGoalState ( stat ) and nextNode:
        # Inicialitzem les variables necessaries.
        node = nextNode.pop (0) # El següent node.
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    [ nextNode.push ( Node ( nStat, node, [nAction], nCost ), nCost + heuristic ( nStat, problem ) ) for nStat, nAction, nCost in problem.getSuccessors (stat) ]
    ...
    while not problem.isGoalState ( stat ) and not nextNode.isEmpty ():
        ...
        if not stat in path and not problem.isGoalState (stat):
            ...
            # Definim els següents passos a seguir.
            [ nextNode.push ( Node ( nStat, node, [nAction], node.cost + nCost ), node.cost + nCost + heuristic (nStat, problem) ) for nStat, nAction, nCost in problem.getSuccessors (stat) if nStat not in path ]
